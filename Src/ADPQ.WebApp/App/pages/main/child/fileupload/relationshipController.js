app.controller('relationshipController', ['$scope', '$rootScope', '$cookieStore', 'helper', '$resource', 'Upload', '$timeout', function ($scope, $rootScope, $cookieStore, helper, $resource, Upload, $timeout) {

    var self = Object.create(null);
    var filename = '';
    $rootScope.RelationShipFileName = [];
    ...
    $scope.uploadFiles = function (errFiles, file) {
        ...
        if (errFiles !== null) {
            ...
            if (errFiles.type !== 'application/pdf') {
                $rootScope.$broadcast('refreshInvalidRelationShipPdf', { name: true });
                self.uploading = false;
                return false;
            }
                ...
                $.ajax({
                    ...
                    success: function (messages) {
                        for (i = 0; i < messages.length; i++) {                         
                            $rootScope.$broadcast('refreshRelationship', { name: messages[i], filename: filename });
                        }
                        self.uploading = false;
                    },
                    error: function () {
